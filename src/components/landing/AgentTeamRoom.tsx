...
const AgentTeamRoom = () => {
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden max-w-[56rem] mx-auto shadow-lg shadow-primary/5">
      {/* Window chrome */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-0 divide-y lg:divide-y-0 lg:divide-x divide-border">
        {/* Agent Roster */}
        <div className="p-5">
          <h4 className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground mb-3.5">
            Team Roster
          </h4>

          {/* Chief of Staff */}
          <div className="flex items-center gap-3 mb-3.5 pb-3.5 border-b border-border">
            <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center shrink-0">
              <Crown className="w-4 h-4 text-primary" />
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-[13px] font-semibold text-foreground">Luna · Chief of Staff</p>
              <p className="text-xs text-muted-foreground">Orchestrates tasks and assigns work</p>
            </div>
            <span className="text-[10px] px-2.5 py-0.5 rounded-full font-medium whitespace-nowrap bg-emerald-500/15 text-emerald-400 transition-all duration-300">
          ...
          <div className="space-y-2.5">
            {agents.map((agent) => (
              <div key={agent.role} className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-semibold text-foreground">{agent.role}</p>
                  <p className="text-xs text-muted-foreground truncate">{agent.description}</p>
                </div>
                <span
                  className={`text-[10px] px-2.5 py-0.5 rounded-full font-medium whitespace-nowrap transition-all duration-300 ${
        ...
        <div className="p-5">
          ...
          <div className="space-y-2.5">
            {feed.map((item, i) => (
              <div key={i} className="bg-surface rounded-lg p-3 transition-all duration-200">
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="flex items-center gap-1.5 bg-secondary px-2 py-0.5 rounded-md shrink-0">
                    <item.agentIcon className="w-3 h-3 text-muted-foreground" />
                    <span className="text-[10px] font-medium text-muted-foreground">{item.agent}</span>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="border-t border-border px-5 py-2.5 flex items-center justify-between">
        ...
        <Button onClick={runCycle} disabled={isAnimating} variant="outline" size="sm" className="gap-1.5 ml-3 shrink-0 h-8 text-xs px-3">

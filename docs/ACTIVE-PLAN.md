...
- **Current validation status (2026-03-07, session 28):** `npx tsc --noEmit` passes for the hardened hotfix. The last completed external QA pass was against the pre-hardening revision, so production redeploy plus one fresh-tenant validation pass are still required before the duplicate-bootstrap bug can be considered closed with high confidence.

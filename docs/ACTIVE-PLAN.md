...
- **Founder-requested live upgrade benefit check completed on `vidacious-4` (2026-03-13, session 68):** Direct runtime probes on the active tenant (`6c6ae22c-d682-4af6-83ff-79913d267aea`, droplet `557399795` / `137.184.56.1`) confirmed OpenClaw `2026.3.11` core health and Slack channel runtime remain good (`/health` live, `channels status` `running:true`, config validate `valid:true`). Dynamic-subagent behavior is actively working: `sessions_spawn` executed with real tool-call/tool-result traces and successful child-session completion, with child session `cwd` inheriting `/home/node/.openclaw/workspace-main`. Memory behavior is also healthy (`memory status`, forced index, and search hit from `memory/active-priorities.md`). Browser-assisted workflows are not improved on this tenant because no browser executable is present after runtime simplification: `browser status` reports no detected browser and `browser start` fails with `No supported browser found`; agent browser probe returns `running=false`. Security hardening from upstream is only partially realized because current config still keeps `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true` (security audit remains `3 critical / 5 warn / 2 info`). Net: upgrade is stable and beneficial for subagent + memory paths, while browser is intentionally unavailable and origin hardening is still held back by local policy config.
- **Memory key validity is now re-verified on a post-rotation fresh canary (2026-03-13, session 67):** Founder rotated Vercel `MEMORY_OPENAI_API_KEY` and redeployed. Re-check on older canary droplet `162.243.160.239` confirmed stale pre-rotation env still failing with OpenAI `401 invalid_api_key` (`...XAAY` fingerprint), but that was expected for an already-provisioned droplet. A new post-rotation canary (`pixelport-dry-run-mmoezocv`, tenant `6640c856-7481-4537-9e20-8413193cb5b4`, droplet `557927762` / `68.183.124.49`) reached `active`, showed updated key fingerprint (`...bXAA`), passed memory auth, and after forced index returned a real hit from `memory/active-priorities.md` for `Complete post-onboarding bootstrap`. Result: onboarding/provisioning and native-memory key-quality blockers are cleared for new tenants.
- **Release merged/deployed; onboarding flow unblocked, memory key quality still blocked (2026-03-13, session 66):** After founder-confirmed CTO approval, branch `codex/vidacious-runtime-permissions-stabilization` was fast-forwarded into `main` and pushed at `71077aa`. GitHub commit status turned `success` and Vercel deployed `https://vercel.com/sanchalrs-projects/pixelport-launchpad/8FYPrT1tB4G9FpJMMU4rDqtjUkjp`. Inngest was re-registered (`PUT /api/inngest` => `modified:true`). Fresh production canary `pixelport-dry-run-mmoeognr` (`8fd36cd0-427c-4985-83d5-197739db0a62`) reached `active` with persisted droplet refs (`557926844` / `162.243.160.239`), proving the onboarding/provisioning hard-fail guard is fixed. Runtime checks on the new droplet passed (`/health` live, container healthy, no `EACCES`/DB-open permission errors in logs). Remaining blocker surfaced by smoke: `openclaw memory search` now fails with OpenAI `401 invalid_api_key`, and direct OpenAI probe confirms the configured key value is invalid; this is an env-quality issue, not a provisioning-flow failure.
- **Onboarding hard-fail guard replaced with graceful memory downgrade (2026-03-13, session 65):** Inngest `Provision New Tenant` failures were traced to a hard throw when `memory_native_enabled=true` but `MEMORY_OPENAI_API_KEY` was absent. Provisioning now resolves an explicit memory runtime plan and continues without crashing: if the key is missing, native memory is disabled for that run only, a durable warning is persisted in `tenants.onboarding_data.provisioning_memory`, and cloud-init/OpenClaw config is emitted with the effective memory state so behavior stays truthful. Targeted tests were added for downgrade/no-downgrade resolution (`api/lib/tenant-memory-settings.ts`, `api/inngest/functions/provision-tenant.ts`, `src/test/tenant-memory-settings.test.ts`), and `vitest` + `tsc` passed.
- **Vidacious-4 runtime stabilization completed (2026-03-13, session 64):** Founder-reported Pixel issues were verified directly on active tenant `vidacious-4` (`6c6ae22c-d682-4af6-83ff-79913d267aea`, droplet `557399795` / `137.184.56.1`). At capture time, memory CLI was failing with `unable to open database file`, and security audit matched the reported host-header fallback/open-group posture findings. The live droplet was repaired in place by normalizing `/home/node/.openclaw` + `{identity,devices}` ownership/perms to `node:node` (`700`) and tightening `/opt/openclaw/{openclaw.json,.env}` to `600`. Post-fix checks passed: gateway health/live endpoint OK, Slack channel runtime still `running:true`, `openclaw memory status --json` now succeeds, forced reindex completed, and memory search returned the expected `memory/active-priorities.md` hit for `Canonical status snapshot recorded`. Security audit dropped from `4 critical · 6 warn` to `3 critical · 5 warn`, clearing the file/state permission findings while leaving intentionally unchanged policy warnings (`dangerouslyAllowHostHeaderOriginFallback`, Slack `groupPolicy:"open"` exposure class). Persistent hardening is now in repo: generated provisioning script enforces host file `600` perms and runs post-start runtime state normalization with retry; template parity and regression tests were updated (`api/inngest/functions/provision-tenant.ts`, `infra/provisioning/cloud-init.yaml`, `src/test/provision-tenant-memory.test.ts`).

...
- **Cleanup follow-up simplified the Vercel fix to protect production deploys (2026-03-12, session 59):** The first cleanup commit on `main` successfully removed the merged remote release branches and the stray local `.playwright-cli/` artifact, but its new `ignoreCommand` logic produced a failed Vercel deployment on commit `45f1430`. Rather than keep iterating on shell logic inside production config, the follow-up cleanup removes `ignoreCommand` from `vercel.json` entirely so future pushes always build. This trades some deploy-cost discipline for safer release behavior and eliminates the same class of false-skip risk that blocked the native-memory release.
